...
calc.maxpos <- function(id, time, align, pos) {
  require(stringr)
  ...
  map.r <- dplyr::bind_cols(do.call(bind_cols, bam[[1]][mapInfo]), do.call(bind_cols, bam[[1]]$tag))
  ...
  countName <- paste("count", id, sep = ".")

  r.sum.max.pos <-
    r.sum.arms %>%
    group_by(arm.name) %>%
    top_n(n = 5, wt = count) %>% ungroup() %>%
    rename_(.dots = setNames("count", countName))
